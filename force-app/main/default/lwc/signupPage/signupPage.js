...
export default class SignupPage extends NavigationMixin(LightningElement) {
    ...
    @track birthday = '';
    @track joiningDate = '';
    @track Manager = '';  // This will hold the selected manager ID
    @track isManager = false;
    @track managerOptions = [];  // Options for the dropdown
    ...
    loadManagerOptions() {
        getManagers()
            .then((data) => {
                // Map the manager data to the required format
                this.managerOptions = data.map(manager => ({
                    label: manager.Name,  // Manager Name displayed in the dropdown
                    value: manager.Id      // Manager Id stored in the dropdown's value
                }));
